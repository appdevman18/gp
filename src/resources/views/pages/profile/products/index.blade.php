                    ...
                    <div class="col-lg-12 margin-tb">
                        <div class="pull-right">
                            ...
                            {{ $isCanCreateProduct ? '' : 'disabled' }}"
                               href="{{ route('profile.products.create') }}">
                                {{ __('Create') }}
                            </a>
                        </div>
                    </div>
                        ...
                        <table class="table table-hover table table-responsive-lg">
                            ...
                            @foreach ($products as $product)
                                <tr>
                                    ...
                                    <td class="col-3">{{ $product->store }}</td>
                                    <td class="col-1">
                                    ...
                                    <td class="col-2">
                                        <form action="{{ route('profile.products.destroy', $product->id) }}"
                                              method="POST">
                                            <a class="btn btn-info btn-sm"
                                               href="{{ route('profile.products.show', $product->id) }}">Show</a>
                                            @role('admin', 'manager')
                                            @csrf
                                            @method('DELETE')
